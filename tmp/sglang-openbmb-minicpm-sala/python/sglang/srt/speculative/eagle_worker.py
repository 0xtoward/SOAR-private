...
from sglang.srt.speculative.sala_trace import (
    root_only_eagle_verify,
    root_only_eagle_use_decode_forward,
    spec_trace_enabled,
    spec_trace_event,
    tensor_snapshot,
)
...
class EAGLEWorker(TpModelWorker):
    ...
    def draft(self, batch: ScheduleBatch):
        ...
        if root_only_eagle_verify() and not batch.forward_mode.is_idle():
            bs = batch.batch_size()
            draft_tokens = spec_info.verified_id.to(
                device=batch.device, dtype=torch.long
            ).reshape(bs)
            positions = batch.seq_lens.to(device=batch.device, dtype=torch.long).clone()
            retrive_index = torch.arange(
                0, bs, dtype=torch.long, device=batch.device
            ).reshape(bs, 1)
            retrive_next_token = torch.full(
                (bs, 1), -1, dtype=torch.long, device=batch.device
            )
            retrive_next_sibling = torch.full(
                (bs, 1), -1, dtype=torch.long, device=batch.device
            )
            # For a root-only verify each request has exactly one query token.
            # The Triton target-verify mask is flattened per request as
            # draft_token_num * (prefix_len + draft_token_num).
            mask_len = int(torch.sum(batch.seq_lens + 1).item())
            tree_mask = torch.full(
                (mask_len,), True, dtype=torch.bool, device=batch.device
            )

            if spec_trace_enabled():
                spec_trace_event(
                    "eagle_root_only_verify_input",
                    {
                        "batch_size": bs,
                        "draft_token_num": 1,
                        "draft_token": tensor_snapshot(draft_tokens),
                        "positions": tensor_snapshot(positions),
                        "seq_lens": tensor_snapshot(batch.seq_lens),
                        "custom_mask_numel": mask_len,
                    },
                )
    ...
    def verify(self, batch: ScheduleBatch, spec_info: EagleVerifyInput):
        ...
        spec_info.num_tokens_per_batch = (
            spec_info.draft_token_num
            if root_only_eagle_verify()
            else self.speculative_num_steps + 1
        )
        batch.return_hidden_states = False
        use_decode_forward_for_root_only = (
            root_only_eagle_use_decode_forward()
            and root_only_eagle_verify()
            and spec_info.draft_token_num == 1
            and not batch.forward_mode.is_idle()
        )
        batch.forward_mode = (
            ForwardMode.DECODE
            if use_decode_forward_for_root_only
            else ForwardMode.TARGET_VERIFY
            if not batch.forward_mode.is_idle()
            else ForwardMode.IDLE
        )
        # In the diagnostic root-only decode-forward path, the target model must
        # see an ordinary decode batch.  Keep the local `spec_info` for verify
        # and commit, but do not expose it to attention metadata construction.
        batch.spec_info = None if use_decode_forward_for_root_only else spec_info

        if spec_trace_enabled() and use_decode_forward_for_root_only:
            spec_trace_event(
                "eagle_root_only_decode_forward_override",
                {
                    "draft_token_num": int(spec_info.draft_token_num),
                    "seq_lens": tensor_snapshot(batch.seq_lens),
                    "input_ids": tensor_snapshot(batch.input_ids),
                    "positions": tensor_snapshot(spec_info.positions),
                    "out_cache_loc": tensor_snapshot(batch.out_cache_loc),
                },
            )
        decode_forward_seq_lens_backup = None
        decode_forward_seq_lens_cpu_backup = None
        decode_forward_seq_lens_sum_backup = None
        if use_decode_forward_for_root_only:
            # Normal decode prepares the batch by advancing seq_lens before the
            # model forward, so ForwardBatch.positions=clamp(seq_lens) points at
            # the token being written.  TARGET_VERIFY does not advance seq_lens
            # until commit.  For this diagnostic decode-forward path, temporarily
            # match normal decode metadata, then restore the pre-verify view before
            # EagleVerifyInput.verify() performs the normal speculative commit.
            decode_forward_seq_lens_backup = batch.seq_lens.clone()
            decode_forward_seq_lens_cpu_backup = batch.seq_lens_cpu.clone()
            decode_forward_seq_lens_sum_backup = batch.seq_lens_sum
            batch.seq_lens.add_(1)
            batch.seq_lens_cpu.add_(1)
            batch.seq_lens_sum += batch.batch_size()

        model_worker_batch = batch.get_model_worker_batch(
            seq_lens_cpu_cache=(
                batch.seq_lens_cpu if use_decode_forward_for_root_only else spec_info.seq_lens_cpu
            )
        )
        if use_decode_forward_for_root_only:
            # batch.spec_info is intentionally hidden from attention metadata in
            # this diagnostic path, so get_model_worker_batch cannot infer that
            # EAGLE still needs target hidden states for draft/cache update.
        ...
        if use_decode_forward_for_root_only:
            ...
            batch.seq_lens = decode_forward_seq_lens_backup
            batch.seq_lens_cpu = decode_forward_seq_lens_cpu_backup
            batch.seq_lens_sum = decode_forward_seq_lens_sum_backup
        ...
        if use_decode_forward_for_root_only and logits_output.hidden_states is None:
            raise RuntimeError(
                "SGLANG_EAGLE_ROOT_ONLY_USE_DECODE_FORWARD requires target "
                "hidden states. The diagnostic path must not fall back to "
                "uninitialized hidden states because EAGLE draft/cache update "
                "may consume them."
            )
        ...
        if (
            ...
            if not use_decode_forward_for_root_only:
                self._mamba_verify_update(
                    batch, res, logits_output, spec_info, seq_lens_pre_verify
                )
            elif spec_trace_enabled():
                spec_trace_event(
                    "eagle_root_only_decode_forward_skip_mamba_verify_update",
                    {
                        "reason": "decode_forward_already_committed_temporal_state",
                        "draft_token_num": int(spec_info.draft_token_num),
                        "accepted_indices": tensor_snapshot(res.accepted_indices),
    ...
    def _mamba_verify_update(
        ...
        if accepted_steps.numel() > 0:
            invalid_steps = (accepted_steps < 0) | (
                accepted_steps >= batch.spec_info.draft_token_num
            )
            if torch.any(invalid_steps):
                raise RuntimeError(
                    "Invalid accepted SimpleGLA verify step inferred from EAGLE "
                    f"accepted_indices. accepted_steps={accepted_steps.tolist()}, "
                    f"draft_token_num={batch.spec_info.draft_token_num}, "
                    f"accepted_indices={res.accepted_indices.tolist()}"
                )
        ...
        if spec_trace_enabled():
            spec_trace_event(
                "eagle_mamba_verify_update",
                {
                    "is_minicpm_hybrid": bool(
                        self.target_worker.model_runner.minicpm_hybrid_config
                        is not None
                    ),
                    ...
                    "accepted_indices": tensor_snapshot(res.accepted_indices),
                    "accept_length_per_req_cpu": res.accept_length_per_req_cpu,
                    "accepted_steps": tensor_snapshot(accepted_steps),
                    "seq_lens_pre_verify": tensor_snapshot(seq_lens_pre_verify),
                    "seq_lens_after_verify": tensor_snapshot(batch.seq_lens),
                    "mamba_steps_to_track": tensor_snapshot(mamba_steps_to_track),
                },
            )
