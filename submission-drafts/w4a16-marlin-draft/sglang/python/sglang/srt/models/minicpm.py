...
class MiniCPMAttention(nn.Module):
    def __init__(
        ...
        output_gate_quant_config = quant_config
        if quant_config is not None and quant_config.get_name() in ("gptq", "gptq_marlin"):
            output_gate_quant_config = None
        ...
        if self.use_output_gate:
            self.o_gate = ColumnParallelLinear(
                ...
                self.total_num_heads * self.head_dim,
                bias=False,
                quant_config=output_gate_quant_config,
...
class MiniCPMModel(nn.Module):
    ...
    def load_kv_cache_scales(self, quantization_param_path: str) -> None:
        ...
        full_attn_layers = {
            idx
            for idx, mixer_type in enumerate(self.config.mixer_types)
