...
soar_uv pip install --python "${QUANT_PY}" --upgrade pip "setuptools>=78.1.1" wheel packaging "cmake>=3.26"
soar_require_torch "${QUANT_PY}" "quant env"
soar_uv pip install --python "${QUANT_PY}" --no-deps "${SOAR_FLASH_ATTN_WHEEL}" "${SOAR_GPTQMODEL_WHEEL}"
soar_uv pip install --python "${QUANT_PY}" --no-deps "ninja"

soar_uv pip install --python "${QUANT_PY}" --no-deps \
  "transformers==5.5.0" \
  "accelerate==1.13.0" \
  "optimum==2.1.0" \
  "sentencepiece==0.2.1" \
...
soar_uv pip install --python "${QUANT_PY}" --no-deps -e "${SCRIPT_DIR}/sglang/python"
...
run_attempt() {
  ...
  if [[ -n "${dynamic_config_path}" ]]; then
    dynamic_arg=("--dynamic-config-path" "${dynamic_config_path}")
  fi
...
run_attempt \
  "publichybrid_attnqkv_allqkv189_bf16full115_mse24" \
  "${SCRIPT_DIR}/calibration_gptq_w4a16_publichybrid_semanticend49k_reduced_v1.jsonl" \
  "115" \
  "bfloat16" \
  "${SCRIPT_DIR}/configs/selective_marlin/attnqkv-allqkv189-bf16full115-mse24.json" \
  --offload-to-disk \
  --vram-strategy exclusive \
  --gc-mode on_stage_end \
  --cpu-cache-layer-outputs && exit 0

echo "[prepare_model] all-QKV189 bf16full115 experiment failed; stopping without unrelated fallback attempts" >&2
exit 1
