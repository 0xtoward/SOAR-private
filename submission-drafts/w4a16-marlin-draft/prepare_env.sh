...
uv pip install --python "${EVAL_PY}" --upgrade pip setuptools wheel packaging
soar_require_torch "${EVAL_PY}" "eval env"
uv pip install --python "${EVAL_PY}" --no-deps "${SOAR_FLASH_ATTN_WHEEL}"

uv pip install --python "${EVAL_PY}" --no-deps \
  "transformers==4.57.1" \
  "accelerate==1.13.0" \
  "sentencepiece==0.2.1" \
...
uv pip install --python "${EVAL_PY}" --no-deps -e "${SCRIPT_DIR}/sglang/python"
...
export SGLANG_SERVER_ARGS="--disable-radix-cache --attention-backend flashinfer --force-dense-minicpm --chunked-prefill-size 8192 --skip-server-warmup --disable-cuda-graph --quantization gptq_marlin --dtype float16"
