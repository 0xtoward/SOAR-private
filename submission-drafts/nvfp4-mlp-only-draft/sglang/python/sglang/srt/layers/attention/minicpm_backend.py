...
class MiniCPMSparseBackend(AttentionBackend):
    ...
    def __init__(
        ...
        self.max_fused_batch_size = model_runner.server_args.max_running_requests
        if self.fuse_topk and self.max_fused_batch_size is None:
            print(
                "Warning: disabling fuse_topk because max_running_requests is unset. "
                "Pass --max-running-requests to prebuild fused kernels."
            )
        ...
        # The local TileLang fork requires these signature arguments even though
        # the current kernel body does not reference them.
        prefill_actual_max_seqlen_q = model_runner.server_args.chunked_prefill_size
        prefill_actual_max_seqlen_k = max_cache_len
        ...
        if self.fuse_topk:
            print("jit start...")
            for bs in range(1, self.max_fused_batch_size + 1):
                ...
                prefill_kernel = fused_attn_pooling_online_topk_prefill(
                    ...
                    pooled_k_len=bucketed_pooled_k_len,
                    actual_max_seqlen_q=prefill_actual_max_seqlen_q,
                    actual_max_seqlen_k=prefill_actual_max_seqlen_k,
                    m_block_dim=16,
                    block_stride=pooling_block_stride,
                    pad_len=pooling_pad_len,
                    num_offs=pooling_num_offs,
