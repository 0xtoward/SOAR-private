...
class HybridLinearAttnBackend(AttentionBackend):
    ...
    def update_simple_gla_state_after_target_verify(
        ...
        source_states = (
            intermediate_state_cache[:, src_state_indices, last_steps]
            if src_state_indices.numel() > 0
            else None
        )

        if spec_trace_enabled():
            spec_trace_event(
                "simple_gla_commit_before",
                {
                    "accepted_steps": tensor_snapshot(accepted_steps),
                    "dst_state_indices": tensor_snapshot(dst_state_indices),
                    "src_state_indices": tensor_snapshot(src_state_indices),
                    "last_steps": tensor_snapshot(last_steps),
                    "temporal_before": tensor_checksum(
                        temporal_states[:, dst_state_indices, :]
                        if dst_state_indices.numel() > 0
                        else None
                    ),
                    "intermediate_source": tensor_checksum(
                        source_states
                    ),
                },
            )
        ...
        if source_states is not None:
            temporal_states[:, dst_state_indices, :] = source_states.to(
                temporal_states.dtype, copy=False
            )

        if spec_trace_enabled():
            commit_source_diff = None
            if source_states is not None:
                diff = (
                    temporal_states[:, dst_state_indices, :].float()
                    - source_states.float()
                ).abs()
                commit_source_diff = {
            ...
            spec_trace_event(
                "simple_gla_commit_after",
                {
                    "accepted_steps": tensor_snapshot(accepted_steps),
                    "dst_state_indices": tensor_snapshot(dst_state_indices),
                    "temporal_after": tensor_checksum(
                        temporal_states[:, dst_state_indices, :]
                        if dst_state_indices.numel() > 0
                        else None
                    ),
                    "commit_source_diff": commit_source_diff,
                },
            )
...
class SimpleGLAAttnBackend(MambaAttnBackendBase):
    ...
    def _build_tree_parent_tokens(
        ...
        if spec_trace_enabled():
            spec_trace_event(
                "simple_gla_parent_tokens",
                {
                    "batch_size": int(batch_size),
                    "seq_len": int(seq_len),
                    "retrieve_next_token": tensor_snapshot(retrieve_next_token),
                    "retrieve_next_sibling": tensor_snapshot(retrieve_next_sibling),
                    "retrieve_parent_token": tensor_snapshot(parent_tokens),
                },
            )
    ...
    def forward(
        ...
        if forward_batch.forward_mode.is_target_verify():
            ...
            trace_max_steps = int(
                os.environ.get("SGLANG_SALA_SPEC_TRACE_MAX_STEPS", "8")
            )
            if spec_trace_enabled():
                spec_trace_event(
                    "simple_gla_target_verify_begin",
                    {
                        ...
                        "mamba_indices": tensor_snapshot(mamba_indices),
                        "seq_lens": tensor_snapshot(forward_batch.seq_lens),
                        "positions": tensor_snapshot(
                            getattr(forward_batch, "positions", None)
                        ),
                        "draft_token_num": int(
                            getattr(forward_batch.spec_info, "draft_token_num", -1)
                        ),
                        "parent_tokens": tensor_snapshot(parent_tokens),
                        "initial_state": tensor_checksum(initial_state),
                        "q_step0": tensor_checksum(q_steps[:, :1]),
                        "k_step0": tensor_checksum(k_steps[:, :1]),
                        "v_step0": tensor_checksum(v_steps[:, :1]),
                    },
                )
            batch_indices = torch.arange(
            ...
            for step in range(seq_len):
                ...
                if spec_trace_enabled() and step < trace_max_steps:
                    spec_trace_event(
                        "simple_gla_target_verify_step",
                        {
                            "layer_id": int(layer_id),
                            "cache_idx": int(cache_idx),
                            "step": int(step),
                            "parent_step": tensor_snapshot(
                                parent_tokens[:, step]
                                if parent_tokens is not None
                                else None
                            ),
                            "step_state": tensor_checksum(step_state),
                        },
                    )
                ...
                if spec_trace_enabled() and step < trace_max_steps:
                    spec_trace_event(
                        "simple_gla_target_verify_step_done",
                        {
                            "layer_id": int(layer_id),
                            "cache_idx": int(cache_idx),
                            "step": int(step),
                            "state": tensor_checksum(state),
                        },
                    )
        ...
        if final_state is not None:
            ...
            if cache_idx is not None:
                layer_cache = self.req_to_token_pool.mamba_pool.mamba2_layer_cache(cache_idx)
                is_target_verify = forward_batch.forward_mode.is_target_verify()
                # TARGET_VERIFY may evaluate rejected/tree nodes.  The real
                # request state must be committed later from the accepted step,
                # not from the last node visited by this forward.
                skip_target_verify_temporal_write = is_target_verify and (
                    os.environ.get("SGLANG_SALA_WRITE_TARGET_VERIFY_TEMPORAL", "0")
                    != "1"
                    or os.environ.get(
                        "SGLANG_SALA_SKIP_TARGET_VERIFY_TEMPORAL_WRITE", "0"
                    )
                    == "1"
                )
                if spec_trace_enabled() and forward_batch.forward_mode.is_target_verify():
                    spec_trace_event(
                        "simple_gla_forward_temporal_write",
                        {
                            "layer_id": int(layer_id),
                            "cache_idx": int(cache_idx),
                            "mamba_indices": tensor_snapshot(mamba_indices),
                            "temporal_before": tensor_checksum(
                                layer_cache.temporal[mamba_indices, :]
                            ),
                            "final_state": tensor_checksum(final_state),
                            "skipped": bool(skip_target_verify_temporal_write),
                        },
                    )
